...
**Note that**: the installation model requires a package name for installation. We use `basicsr_lite` as the package name.
...
An example of this dataset is in [basicsr_lite/data/example_dataset.py](basicsr_lite/data/example_dataset.py). It has the following steps.
...
An example of architecture is in [basicsr_lite/archs/example_arch.py](basicsr_lite/archs/example_arch.py). It mainly builds the network structure.
...
An example of model is in [basicsr_lite/models/example_model.py](basicsr_lite/models/example_model.py). It mainly builds the training process of a model.
...
The whole training pipeline can reuse the  [basicsr_lite/train.py](basicsr_lite/train.py), which can be very concise:
...
After the model is trained, it's time for the testing and evaluation phase, which is also conducted based on the configuration file provided in the options folder:

```bash
python basicsr_lite/test.py -opt options/test/Example/test_example.yml
```

The outputs will be saved to `results` folder.

The trained model may also be directly used in practical applications, that is, for inference only, to obtain the model's output without going through the evaluation of the results and other metrics:
...
So far, you have finished developing your own projects using `BasicSR_Lite`. Isn't it very convenient~ :grin:

## As a Template

You can use BasicSR-Lite as a template for your project. Here are some modifications you may need.

1. Set up the *pre-commit* hook
    1. In the root path, run: `pre-commit install`
2. Modify the `LICENSE`<br>
    This repository uses the *MIT* license, you may change it to other licenses

As the installation mode requires the package name, you also need to modify all the `basicsr_lite` names to YOUR_PACKAGE_NAME.
Here are the detailed locations that contain the `basicsr_lite` name:
1. The `basicsr_lite` folder
2. [setup.py](setup.py#L9); &emsp; [setup.py](setup.py#L48); &emsp;[setup.py](setup.py#L91)
3. [basicsr_lite/train.py](basicsr_lite/train.py#L4-L6)
4. [basicsr_lite/archs/\_\_init\_\_.py](basicsr_lite/archs/__init__.py#L11)
5. [basicsr_lite/data/\_\_init\_\_.py](basicsr_lite/data/__init__.py#L11)
6. [basicsr_lite/models/\_\_init\_\_.py](basicsr_lite/models/__init__.py#L11)
