...
Specifically, we implement it through `importlib` and `getattr`. Taking the data module as example, we follow the below steps in [`data/__init__.py`](../basicsr_lite/data/__init__.py):
...
_dataset_modules = [
    importlib.import_module(f'basicsr_lite.data.{file_name}')
...
| Model          | `_model.py`     | `basicsr_lite/models/sr_model.py` |
| Archs          | `_arch.py`      | `basicsr_lite/archs/srresnet_arch.py`|
...
| :------------- | :----------:    | :----------:   |
| Losses           | `basicsr_lite/models/losses`   | [`basicsr_lite/models/losses/__init__.py`](../basicsr_lite/models/losses/__init__.py) |
| Metrics          | `basicsr_lite/metrics`     | [`basicsr_lite/metrics/__init__.py`](../basicsr_lite/metrics/__init__.py)|
...
1. When logging, the loss items are recommended to start with `l_`, so that all these loss items will be grouped together in tensorboard. For example, in [basicsr_lite/models/srgan_model.py](../basicsr_lite/models/srgan_model.py), we use `l_g_pix`, `l_g_percep`, `l_g_gan`, etc for loss items. In [basicsr_lite/utils/logger.py](../basicsr_lite/utils/logger.py), these items will be grouped together:
