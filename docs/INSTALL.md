...
&emsp;deformable convolution: [*dcn* for EDVR](basicsr_lite/ops) (For torchvision>=0.9.0, we use the official `torchvision.ops.deform_conv2d` instead)<br>
&emsp;StyleGAN customized operators: [*upfirdn2d* and *fused_act* for StyleGAN2](basicsr_lite/ops)<br>
you also need to:
...
- If you want to **compile C++ extensions during installation**, please set the environment variable `BASICSR_EXT=True`:
  ...
  ```bash
  BASICSR_EXT=True pip install basicsr_lite
  ```
  ...
  ```bash
  BASICSR_EXT=True pip install basicsr_lite -vvv
  ```
